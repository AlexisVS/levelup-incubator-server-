...
<form action="/incubator/startups/{{ $startup->id }}/goals/{{ $goal->id }}/" method="POST" enctype="multipart/form-data">
  ...
  <div class="px-4 py-3 mb-8 bg-white rounded-lg shadow-md dark:bg-gray-800">
    ...
    <label class="block py-2 text-sm">
      ...
      <input value="{{ $goal_task_template->name }}" type="text" name="name" class="block w-full mt-1 text-sm dark:border-gray-600 dark:bg-gray-700 focus:border-purple-400 focus:outline-none focus:shadow-outline-purple dark:text-gray-300 dark:focus:shadow-outline-gray form-input" placeholder="First name">
    </label>
